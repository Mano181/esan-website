...
export default function CategoryList({ initialCategories }: { initialCategories: Category[] }) {
    const { t } = useLanguage();
    const productCategories = initialCategories.filter(c => c.type === 'category');
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">
                    {productCategories.map((category) => (
                        ...
                        >
                            <div className="w-16 h-16 bg-brand-50 rounded-full flex items-center justify-center mb-3 text-brand-600 relative overflow-hidden">
                                {category.image ? (
                                    <Image
                                        src={category.image}
                                        alt={t(`data.categories.${category.id}.name`)}
                                        fill
